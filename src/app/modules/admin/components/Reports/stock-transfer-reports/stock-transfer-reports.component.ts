...
@Component({
  selector: 'app-stock-transfer-reports',
  templateUrl: './stock-transfer-reports.component.html',
  styleUrls: ['./stock-transfer-reports.component.scss'],
})
export class StockTransferReportsComponent implements OnInit, AfterViewInit {
  inventory: any[] = [];
  ...
  fetchInventoryForSelectedShift() {
    if (this.selectedShiftId !== null) {
      this.adminService
        .getStockTransferOrdersForAShift(this.selectedShiftId)
        .subscribe((data: any) => {
          this.isShiftReports = true;
          this.loading = false;
          this.inventory = data.stockTransfer;
  ...
  fetchInventoryForSelectedRange(startDate: Date, endDate: Date) {
    if (this.startDate && this.endDate) {
      ...
      if (this.selectedShiftId !== null) {
        this.adminService
          .getStockTransfeOrdersForSelectedTimeRange(startDate, endDate)
          .subscribe((data: any) => {
            this.isShiftReports = false;
            this.loading = false;
            this.selectedTimeReports = data;
            console.log('Data for the shift', this.selectedTimeReports);
          });
      }
    }
  }
  ...
  onShiftSelectionChange() {
    ...
    this.endDate = null;
    this.startDate = null;
    this.currentShift = this.shifts.find(
      (data: any) => data.id === this.selectedShiftId
    );
  ...
  printReport() {
    ...
    if (printWindow) {
      const content = `
        <html>
          <head>
            <title>Transferd Items Report</title>
            <style>
              body {
              ...
              <h2>Transfered Items From the Store</h2>
              <table>
                <thead>
                  <tr>
                    <th>Item Name</th>
              ...
              <div class="footer">
                Stock Transfer Rerport Generated On ${new Date().toLocaleDateString()}
              </div>
            </div>
          </body>
        </html>
